...
ssh -o BatchMode=yes "$HOST" "sleep 8; sudo docker compose -f $APP_DIR/docker-compose.yml logs --since 30s 2>&1 | grep -E 'is running \(polling\)|Traceback|Error' | tail -3"
